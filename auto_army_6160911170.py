#!/usr/bin/env python3
"""
🤖 AUTOMATIC PIN HUNTER ARMY - Booking ID: 6160911170
...
HUNTER_SCRIPT = "real_pin_hunter_6160911170.py"
WAIT_BETWEEN_AGENTS = 3  # seconds between agents (stealth mode)
...
def display_agent_launch(agent_num, total_attempts):
    ...
    print(f"\n{Fore.GREEN}🥷 DEPLOYING STEALTH AGENT #{agent_num:03d}{Style.RESET_ALL}")
    ...
    print(f"{Fore.MAGENTA}   Stealth mode: Proxy phantomnet1_ONP6z active{Style.RESET_ALL}")
...
╔══════════════════════════════════════════════════════════════════════════════╗
║                                                                              ║
║                  🎉 REAL HUNT MISSION ACCOMPLISHED! 🎉                      ║
║                                                                              ║
║  Target Booking ID: {REAL_BOOKING_ID}                                        ║
...
"""
        ...
        print(f"{Fore.CYAN}📊 STEALTH ARMY STATISTICS:")
        ...
        print(f"Stealth mode: All attempts made through proxy phantomnet1_ONP6z{Style.RESET_ALL}")
...
def main():
    ...
    try:
        while agent_count < MAX_AGENTS:
            ...
            # Launch agent
            try:
                print(f"{Fore.YELLOW}🚀 Launching stealth agent #{agent_count:03d}...{Style.RESET_ALL}")
                
                # Run the hunter script
                result = subprocess.run([PYTHON_PATH, HUNTER_SCRIPT], 
                                      capture_output=False,
                                      text=True,
                                      timeout=600)  # 10 minute timeout per agent (stealth mode)
            ...
            # Wait before next agent (stealth mode)
            if agent_count < MAX_AGENTS:
                print(f"{Fore.BLUE}⏱️ Stealth delay: {WAIT_BETWEEN_AGENTS} seconds before next agent...{Style.RESET_ALL}")
