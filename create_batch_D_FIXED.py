...
print(f"{Fore.CYAN}🚀 BATCH D ARMY GENERATOR - FIXED VERSION 🚀{Style.RESET_ALL}")
...
print(f"{Fore.BLUE}Using CORRECT booking.com URL and proven logic{Style.RESET_ALL}")
print(f"{Fore.RED}🎯 BEGINNING BATCH D FIXED CREATION!{Style.RESET_ALL}")

# Delete wrong D batch files first
print(f"{Fore.YELLOW}🗑️ Cleaning up wrong D batch files...{Style.RESET_ALL}")
for i in range(1, 101):
    wrong_file = f"agent_{i:03d}D.py"
    if os.path.exists(wrong_file):
        os.remove(wrong_file)
        print(f"{Fore.RED}❌ Removed wrong file: {wrong_file}{Style.RESET_ALL}")

print(f"{Fore.CYAN}🎲 Generating correct sequential PIN ranges...{Style.RESET_ALL}")

for i in range(1, 101):
    agent_id = f"{i:03d}D"
    
    # Calculate PIN range for this agent (5 PINs each) - SYNCHRONIZED WITH BATCH A
    ...
    # Generate the agent script with CORRECT PROVEN LOGIC
...
'''
    ...
    print(f"{Fore.GREEN}✅ Created CORRECT: agent_{agent_id}.py{Style.RESET_ALL}")

print(f"{Fore.GREEN}🎉 BATCH D FIXED CREATION COMPLETE!{Style.RESET_ALL}")
print(f"{Fore.CYAN}📂 Files Created: 100 CORRECT agents{Style.RESET_ALL}")
...
print(f"{Fore.BLUE}🌐 URL: https://secure.booking.com/help/confirmation_pin_auth{Style.RESET_ALL}")
print(f"{Fore.GREEN}✅ BATCH D READY FOR DEPLOYMENT WITH CORRECT LOGIC!{Style.RESET_ALL}")
print(f"{Fore.RED}🏆 400-AGENT ARMY NOW TRULY COMPLETE!{Style.RESET_ALL}")
